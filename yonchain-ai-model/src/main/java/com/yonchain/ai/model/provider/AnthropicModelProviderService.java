 ...
 */
@Service
public class AnthropicModelProviderService extends AbstractModelProviderService {
    ...
    @Autowired
    public AnthropicModelProviderService(ModelProviderConfigLoader configLoader) {
    ...
    private void loadProviderConfig() {
        ...
        this.provider = providers.stream()
                ...
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("未找到Anthropic提供商配置"));
    }

    @Override
    protected void doInitialize(Map<String, Object> config) {
        // 在实际应用中，这里应该使用配置参数初始化Anthropic客户端
        // 例如：设置API密钥、代理等
    }
