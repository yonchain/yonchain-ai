 ...
 * 选项处理器注册中心
 * 
 * 统一管理所有级别的ModelOptionsHandler：
 * - 命名空间级：namespace:type (如 "openai:chat")
 * - 模型级：namespace:modelId (如 "openai:gpt-4")  
 * - 类名级：完整类名 (如 "com.yonchain.ai.model.optionshandler.openai.OpenAiChatModelOptionsHandler")
 * - 别名级：自定义别名 (如 "openai-chat-v1")
 */
public class ModelOptionsHandlerRegistry {
    
    // 存储选项处理器，支持多种键格式
    private final ConcurrentHashMap<String, ModelOptionsHandler<? extends ModelOptions>> handlers;
    
    public ModelOptionsHandlerRegistry() {
     ...
     * @param handler 选项处理器
     */
    public void registerHandler(String key, ModelOptionsHandler<?> handler) {
        ModelOptionsHandler<?> existing = handlers.put(key, handler);
        if (existing != null) {
            System.out.println("ModelOptionsHandler replaced for key: " + key);
        } else {
            System.out.println("ModelOptionsHandler registered for key: " + key);
        }
    }
    ...
    public void registerNamespaceHandlerByClass(String namespace, String type, String handlerClass) {
        try {
            ModelOptionsHandler<?> handler = createHandlerByClass(handlerClass);
     ...
     * 
     * @param key 键（支持多种格式）
     * @return ModelOptionsHandler实例
     */
    @SuppressWarnings("unchecked")
    public <T extends ModelOptions> Optional<ModelOptionsHandler<T>> getHandler(String key) {
        ModelOptionsHandler<T> handler = (ModelOptionsHandler<T>) handlers.get(key);
        return Optional.ofNullable(handler);
    }
    
    /**
     * 解析ModelOptionsHandler（带优先级查找和约定兜底）
     * 
     * 查找优先级：
     ...
     * @return ModelOptionsHandler实例，可能为null
     */
    @SuppressWarnings("unchecked")
    public <T extends ModelOptions> ModelOptionsHandler<T> resolveHandler(String namespace, String modelId, String type, String explicitHandlerClass) {
        ...
        Optional<ModelOptionsHandler<T>> modelHandler = getHandler(modelKey);
        ...
        if (explicitHandlerClass != null && !explicitHandlerClass.isEmpty()) {
            Optional<ModelOptionsHandler<T>> explicitHandler = getHandler(explicitHandlerClass);
            ...
            // 如果注册表中没有，尝试动态创建并缓存
            try {
                ModelOptionsHandler<T> handler = (ModelOptionsHandler<T>) createHandlerByClass(explicitHandlerClass);
        ...
        Optional<ModelOptionsHandler<T>> namespaceHandler = getHandler(namespaceKey);
        ...
        if (conventionClass != null) {
            try {
                ModelOptionsHandler<T> handler = (ModelOptionsHandler<T>) createHandlerByClass(conventionClass);
                // 缓存到命名空间级，提高后续查找性能
                registerNamespaceHandler(namespace, type, handler);
        ...
        System.out.println("DEBUG: 未找到ModelOptionsHandler: " + namespace + ":" + modelId + " (type: " + type + ")");
        return null;
    }
     ...
     * 通过类名创建Handler实例
     */
    private ModelOptionsHandler<?> createHandlerByClass(String handlerClass) throws Exception {
        Class<?> clazz = Class.forName(handlerClass);
        return (ModelOptionsHandler<?>) clazz.getDeclaredConstructor().newInstance();
    }
    
    /**
     * 根据约定规则生成Handler类名
     * 
     * 约定规则：com.yonchain.ai.model.optionshandler.{namespace}.{Namespace}{Type}ModelOptionsHandler
     * 
     * @param namespace 命名空间
     * @param type 模型类型
    ...
    private String generateConventionHandlerClass(String namespace, String type) {
        ...
        return String.format("com.yonchain.ai.model.optionshandler.%s.%s%sModelOptionsHandler", 
